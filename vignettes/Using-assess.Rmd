...
see: [Postestimation: Testing a model](https://m-e-rademaker.github.io/cSEM/articles/Using-test.html)). 
...
we will refer to a generic (fit) index, quality/assessment measure as 
a **quality criterion**.
...
- Convergent and discriminant validity assessment:
    - The **average variance extracted** (AVE) (the basis for the 
      Fornell-Larcker criterion (FL criterion))
...
:  A character string or a vector of character strings naming the quality criterion
   ...
   See the [Details](#details) section below for a list of possible candidates.
  
`.only_common_factors`

:  Logical. Should the quality criterion be calculated for constructs modeled as 
   common factors only? Defaults to `TRUE`.
...
:  Further arguments passed to functions called by `assess()`.
   ...
   - `.alpha` Accepted by/Passed down to: `calculateRhoT()`
   - `.closed_form_ci` Accepted by/Passed down to: `calculateRhoT()`
   - `.saturated` Accepted by/Passed down to: `calculateSRMR()`, `calculateDG()`, 
     `calculateDL()`, `calculateDML()` and subsequently `fit()`.
   - `.type_vcv` Accepted by/Passed down to: `calculateSRMR()`, `calculateDG()`, 
     `calculateDL()`, `calculateDML()` and subsequently `fit()`.
...
- All reliability measures (congeneric, tau-equivalen (Cronbach alpha))
...
to `.quality_criterion`. By default all quality possible quality criteria 
...
This section provides technical details and relevant formulae. To
understand the notation and terminology used in this section see the
notation and the termionology glossary.
...
### Formula
Using the results and notation derived and defined in the Notation glossary, 
