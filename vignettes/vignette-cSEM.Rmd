...
1. Structural equation modeling (SEM) has been used for over a century
   ...
   we do not seek to impose "our" conventions but merly seek to make communication
   ...
   This vignette therefore provides two glossaries:
      - The [termionology glossary](#terminology) containing each term that we think should be 
         definied and explained.
...
Structural equation modeling (SEM) is about modeling the (causal) relationship 
between concepts (an entity defined by a conceptual definition, 
e.g. "emotional stability", "self-confidence", "customer loyalty") with other 
concepts and/or observable quantities generally referred to as 
indicators, manifest variables or items. Broadly speaking, two modeling 
approaches exist. Each approach entails a set of methods, estimators, test,
...
Assuming there are $J$ constructs and $K$ indicators, the fundamental feature 
of the first approach -- usually called the covariance-based 
or factor-based approach -- is the assumption of the existence of a set of $J$ 
common factors that each serve as a stand-in for one of the $J$ concepts to 
be studied in a sense that each factor is at least partly (causally) responsible
for the manifestations of a set of indicators which are supposed to measure 
the concept in question. The entirety of these measurement relations is 
captured by the measurement model which related indicators to common factors 
according to the researchers theory of how observables are related to the 
concepts in question. The entirety of the relationships between concepts is 
captured by the structural model whose parameters are usually at the center 
of the researchers interest.  Caution is warranted though as the common factor 
and its respective concept are not the same thing. Within the covariance-based 
literature concept, construct, latent variable and its representation the 
common factor have often been used interchangeably (Rigdon, 2012, 2016). 
This will not be the case here and readers are explicitly made aware of the 
fact that constructs are the abstract entity which *may* be modeled by 
a common factor, however, no assertion as to the correctness of this approach 
in terms of "closeness of the common factor and its related concept" are made. 
Parameters in covariance based methods are usually retrieved by 
(full-information) maximum likelihood estimation. Notation follows below.

The second approach is known as the variance-based approach, however, I will use
the term composite-based approach instead for the simple reason that it clearly
names the defining feature of these approaches: the composite.
...
:   Historically latent variable and common factor where used interchangeably. 
    We explicitly do not follow this convention. In the following, the term 
    latent variable is only used to mean ``unobserved variable''. In this 
    respect, both conceptual variables as well as common factors are 
    latent variables.
