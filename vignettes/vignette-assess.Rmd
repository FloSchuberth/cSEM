...
*a statistical test*. Quality in this case is taken to be a catch-all term for
...
- **Congeneric reliability** ($\rho_C$), also know as e.g.: composite reliability,
  construct reliability, (unidimensional) omega, Jöreskog's $\rho$, $\rho_A$,
  or $\rho_B$. Under the assumption of tau-equivalent and/or parallel measurement
  an alternative reliability measures my be derived:
    - **Tau-equivalent reliability** ($\rho_T$), also know as e.g.: Cronbach alpha, alpha, $\alpha$,
      coefficient alpha, Guttman's $\lambda_3$, KR-20. 
  
  A third measure is **Parallel reliability** ($\rho_P$), also know as e.g.: split-half reliability, 
  ...
  are always standarized. Hence, only $\rho_T$ is reported.
     
  WARNING: tau-equivalent (and parallel) measurement reliability
  measures are only consistent for the population reliability under restrictive
  assumptions that are unlikly to hold in practice. We therefore explicitly discourage
  their use as congeneric reliability is virtually always preferable in empirical
  work! In fact, congeneric reliability can be shown to be
  identical to the tau-equivalent/parallel reliability *if their respective assumptions acutally hold*,
  essentially offering robustness against violation of the tau-equivalence and/or
  parallel measurement assumption. See the subsection [Reliability](#reliability)
  in the [Methods & Formula](#methods) section for more details.
- Distance measures
...
For implementation details see the [Methods & Formulaes](#methods) section.
...
:  An object of class `cSEMResults` resulting from a call to `csem()`. Note
   ...
   affect how the function is used. See [Details](#details) below.
   
`.what`

:  A character string or a vector of character strings naming the quality measure 
   to compute. By default all qualitiy measures are computed (`"all"`).
...
ensures a specific structure. It is therefore important to remember that
...
composite model. Consequently, their compuation is supressed by default for 
...
the user deliberatly chooses an approach that yields inconsistent estimates (by
...
Based on the assumptions and definitions above the following quantities necessarly follow:
...
common defintions are given below:
...
$$ \text{AVE} = \frac{\sum Var(\eta_k)}{\sum Var(x_k)} = \frac{\sum\lambda^2_k}{\sum(\lambda^2_k + Var(\varepsilon_k))}$$
...
As an important consequence, the AVE now equals
the average of the $K$ (indicator) reliabilities (Reliability: the squared correlation between
a measure and its related latent variable. See section: [Reliability](#reliability) below.
...
The AVE is the basis for the Fornell-Larcker criterion. See validity assessment.
...
with validity as a perferctly reliable measure may be completly invalid.
...
which provides the underlying justification for the measurement model postulated at the begining
of the [Methods & Formulae](#methods) section. Based on the true score framework
reliability is definied as
...
types of measurment model provide natural naming candidates for their corresponding
...
$$ \rho_C = \frac{Var(\bar\eta)}{Var(\hat\eta_k)} = \frac{(\bm{w}\bm{\lambda})^2}{\bm{w}'\bm{\Sigma}\bm{w}}$$
...
Using the assumptions imposed by the paralell measurement model we obtain
...
\begin{align}
\rho_C &=  (\sum w_k\lambda_k)^2 = (\bm{w}\bm{\lambda})^2\\
...
In pratice, these assumptions are unlikly to hold. We therefore explicitly discourage
their use as congeneric reliability is virtually always preferable in empirical
work! In fact, as shown above, congeneric reliability is identical to the 
tau-equivalent/parallel reliability *if their respective assumptions acutally hold*,
essentially offering robustness against violation of the tau-equivalence and/or
...
particulary true for newcomers to structural equation modeling 
...
possible by relying on a paper by Cho (2016) who propsed, uniform 
...
helps clarify potential missconceptions thus preventing potenital missue, such as
...
the formula and more so its correct application. To facilitate the tranlation between
different naming systems and conventions we provide a "translation table" below:

<center>
--------------------------------------------------------------------------------
 Systematic names             Mathematical Synonomous terms      
          
----------------------------- ------------ -------------------------------------
 Parallel reliability           $\rho_P$   Spearman-Brown formula, Spearman-Brown prophcy,
                                           Standardized alpha, Split-half reliability

 Tau-equivalent reliability     $\rho_T$   Cronbach alpha, $\alpha$, Coefficient alpha
...
#### Implementation
Function are implemented as: `calculateRhoP()`, `calculateRhoT()`, and `calculateRhoC()`. 
