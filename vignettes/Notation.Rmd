...
The structural model specifies the relationships between constructs (i.e. the statistical
representation of the concept) via paths (arrows) and associated path coefficients.
The path coefficients - somethimes also called structural coefficients - express the magnitude of the influence exerted by the 
construct at the start of the arrow on the variable at the arrow's end. 
In composite-based SEM constructs are always operationalized as composites, i.e.,
weighted linear combinations of its respective indicators. Consequently, depending on how
a given construct is modeled, such a composite may either serve as a proxy for
an underlying latent variable (common factor) or as a composite in its own right.
Despite this crucial difference, we stick with the common - although somewhat 
ambivalent - notation and represent both the latent variable and the construct
by $\eta_j$. Let $x_{kj}$ be an indicator (observable) and $\varepsilon_{kj}$ be a measurement error.
...
$$E(\hat\eta_j) = 0\quad\quad \text{and}\quad\quad Var(\hat\eta_j) = 1$$

Since the relations between constructs are a product of the researcher's theory
and assumptions to be analyzed, they do not necessarily exist between all 
variables. Instead, it is likely that some variables are not directly connected 
by a path. If all paths exist, we call the structural model  'saturated' model. 
In the case that some paths are not specified, the structural model is called 
'non-saturated'. Whether the structural model is saturated or not can have an 
influence on the model fit when variables or indicators are 'falsely' missing 
in the model specification (that means in comparison to the 'true' model).

Suppose that there are constructs $\eta_k$ and $\eta_l$ with a path 
leading from $\eta_k$ to $\eta_l$ with no other latent variable having an
...
accountig for the specific associated paths, the variance covariance matrix of 
