...
print.cSEMTestOMF <- function(x, ...) {
  ...
  cat(
    ...
    ansi_align("", width = 14), 
    "\t",
    ansi_align("Critical value", width = 8*ncol(x$Critical_value), 
              align = "center"),
    sep = ""
  )
  cat(
  ...
  for(i in colnames(x$Critical_value)) {
    cat(ansi_align(i, width = 6, align = "center"), "\t", sep = "")
  }
  ...
  for(j in seq_along(x$Test_statistic)) {
    cat(
      ansi_align(names(x$Test_statistic)[j], width = 20),
      ansi_align(sprintf("%.4f", x$Test_statistic[j]), width = 14, 
                align = "center"), 
      "\t", 
      sep = ""
    )
  ...
  for(i in colnames(x$Critical_value)) {
    cat(ansi_align(i, width = l[i], align = "center"), "\t", sep = "")
  }
  ...
  for(j in seq_along(x$Test_statistic)) {
    ...
    cat(ansi_align(names(x$Test_statistic)[j], width = 20), "\t", sep = "")
    
    for(k in 1:ncol(x$Critical_value)) {
      cat(
        ansi_align(ifelse(x$Decision[j, k], 
                         col_green("Do not reject"), col_red("reject")), 
