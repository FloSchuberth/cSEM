...
calculateHTMT2 <- function(
  ...
  htmtgeo<-sapply(block_pairs,function(x){
    # calculate geometric mean for each block
    monotraits<-sapply(x,function(y){
      cor_temp<-S[y,y]
      psych::geometric.mean(cor_temp[lower.tri(cor_temp)])
    })
    mono<-psych::geometric.mean(monotraits)
    heterotraits<-psych::geometric.mean(c(S[x[[1]],x[[2]]]))
    heterotraits/mono
  })
