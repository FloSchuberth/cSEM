...
print.testMICOM <- function(.object) {
  ...
  construct_names <- rownames(x2$Decision[[1]])
  l <- max(nchar(c("Construct", construct_names)))

    for(i in seq_along(x2$Test_statistic)) {
i <- 1
    cat("Groups: ", names(x2$Test_statistic)[i], "\n\t",
        col_align("", l + 2, align = "center"),
        col_align("", 10, align = "center"), "\t",
        col_align("Critical Value(s)", 8*ncol(x2$Critical_value[[1]]), align = "center"), "\n\t",
        col_align("Construct", l + 2, align = "center"),
        col_align("Test statistic (c)", 10, align = "center"), "\t",
        sep = "")
