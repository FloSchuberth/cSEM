...
parse_GSCAPro_FullResults <-
  ...
  headers = c(
    ...
    browser()  
    # tables <- lapply(headers, FUN = \(x) try(data.table::fread(file = file, fill = TRUE, skip = x, sep = ",",
    #                                                            blank.lines.skip = TRUE)))
    
    file <- here::here(file_path)  
    
    to_parse <- data.table::fread(file = file, fill = TRUE, sep = ",")
    ...
    list_of_blanks <- lapply(idx_of_headers, FUN = \(x) which.max(x < idx_of_blank_rows))
    idx_first_blank_after_header <- lapply(list_of_blanks, FUN = \(x) idx_of_blank_rows[x]) |>
      unlist()
    ...
    tables <- mapply(
      ...
      file = file,
      fill = TRUE,
      skip = idx_of_headers+1, # Add one because in Version 1.2.1 of GSCA Pro the Header was one row ahead of the column labels
      nrows = nrows_of_tables, # Minus one for adjusting for the idx_of_headers and another for the empty-gaps between tables
      sep = ","
    )
