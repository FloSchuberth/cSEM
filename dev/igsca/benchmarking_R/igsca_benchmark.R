...
if (FALSE) {
  # jarl-ignore unreachable_code: just for reference
  ...
  (single_indicator_weight <- 0.4 / sqrt(0.4 %*% 1 %*% 0.4))
}
...
bigN_smallp <- function(.dgp = dgp_smallp, .mod = mod_smallp) {
  set.seed(42)
  dat <- cSEM.DGP::generateData(.dgp, .empirical = TRUE, .N = 10000)
  csem(
    .data = dat,
    .model = .mod,
...
smallN_bigp <- function(.dgp = dgp_bigp, .mod = mod_bigp) {
  set.seed(42)
  # dat <- cSEM.DGP::generateData(dgp_bigp, .empirical = TRUE, .N = 50)
  dat <- cSEM.DGP::generateData(.dgp, .empirical = TRUE, .N = 50)
  csem(
    .data = dat,
    .model = .mod,
