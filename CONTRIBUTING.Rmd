...
The package structure is best understood with reference to a (hierachically organized) company. In analogy to company departments, two separate "function departments" 
exist:

- **Estimation functions**
- **Postestimation functions**

Each department is hierachically structured. All functions that do not belong
to one of the departements are considered **utility functions**. To stay in the
picture, they are best understood as external consultants in charge of one
specific task that is not meaningfully classified as beloning to one of the two
departements.

#### Estimation functions:

Estimation functions are all functions involed in the estimation/calculation 
...
- **Lowlevel (helper) functions** that perform one specifc task. A distinction
  is made between:
...
Therefore postestimation functions are generally also hierachical. 
...
1. Verbs in function names should be consistent across the whole package. Avoid to
   mix synonomys. Example: `computeValue()` vs. `calculateValue()`.
   This package always uses `calculate` instead of `compute`. Similarily, `method` 
...
1. Strive for meaningful argument names even if they are a bit longer than ususal.
   ...
   For example: any argument das describs a method or an approach should be named
   `.appraoch_*`.
...
Flexibilty will come at the price of code repetition (i.e. most exported helper 
...
1. Add the new argument + a description to the cSEMArguments list by writing
  `@param <argument> Description. Default.`
1. Add the argument to the `args`-list in `args_default()` and provide a
   default value. 
